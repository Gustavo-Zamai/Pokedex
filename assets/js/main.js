...
prevPageBtn.addEventListener('click', () => {
    offset -= limit;
    window.scrollTo(0, 0);
    if (nextPageBtn.classList.contains('hidden')) nextPageBtn.classList.remove('hidden');
    loadPokemonsItens(offset, limit);
});
...
function loadPokemonsItens(offset, limit) {
    pokeApi
        ...
        .then((pokemons = []) => {
            const newList = pokemons
                .map(pokemon => {
                    return `
                    <li onClick="getInfoPokemon(${pokemon.id}, '${pokemon.name}', '${pokemon.types
    ...
    offset == 0 ? prevPageBtn.classList.add('hidden') : prevPageBtn.classList.remove('hidden');
}
